...
export default function BrowsePlaylistsPage() {
    ...
    const fetchAllContent = useCallback(async () => {
        ...
        try {
            ...
            setLoadError("Couldn't load playlists. Check your connection and try again.");
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    const handleUrlSubmit = async () => {
        ...
        try {
            const response = await api.post<{ source: string; id: string; url: string }>(
                "/browse/playlists/parse",
                { url: urlInput.trim() }
            );
            setShowUrlModal(false);
            setUrlInput("");
            router.push(`/import/spotify?url=${encodeURIComponent(response.url)}`);
        } catch (error: unknown) {
    ...
    const renderCard = (item: PlaylistPreview, index: number, context?: string) => (
        <div
    ...
    const renderGenreCard = (genre: Genre) => (
        ...
        >
            ...
            <div className="absolute bottom-3 left-3 right-3">
                <h3 className="text-sm font-black text-white uppercase tracking-tight">{genre.name}</h3>
            </div>
        ...
        <div className="min-h-screen bg-gradient-to-b from-[#0a0a0a] to-black relative">
            ...
            <div className="relative px-4 md:px-8 pb-24">
                <div className="max-w-[1800px] mx-auto">
                    ...
                    <div className="animate-slide-up mb-6" style={{ animationDelay: "0s" }}>
                        <div className="flex items-start gap-3 px-4 py-3 rounded-lg bg-[#a855f7]/5 border border-[#a855f7]/20">
                            ...
                            <p className="text-xs font-mono text-white/50 leading-relaxed">
                                <span className="font-black text-[#a855f7] uppercase tracking-wider">Beta</span>{" "}
                    ...
                    <div className="animate-slide-up flex items-center gap-3 mb-6" style={{ animationDelay: "0.1s" }}>
                    ...
                    {!isLoading && !isSearching && hasSearched && (
                        <div className="animate-slide-up" style={{ animationDelay: "0s" }}>
                            <SectionHeader title={`Results for "${searchQuery}"`} count={playlists.length} />
                            {playlists.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-20 text-center">
                                    <h3 className="text-lg font-black text-white mb-2 tracking-tight">
                                        No playlists found
                    ...
                    {!isLoading && selectedGenre && (
                        <div className="animate-slide-up" style={{ animationDelay: "0s" }}>
                            <SectionHeader title={`${selectedGenre.name} Playlists`} count={genrePlaylists.length} />
                    ...
                    {/* Main Content */}
                    {!isLoading && !isSearching && !hasSearched && !selectedGenre && (
                        <>
                            {activeTab === "playlists" && (
                                <div className="animate-slide-up" style={{ animationDelay: `${(sectionIndex++) * 0.1}s` }}>
                                    <SectionHeader title="Featured Playlists" count={playlists.length} />
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-6">
                            ...
                            {activeTab === "genres" && (
                                <div className="animate-slide-up" style={{ animationDelay: `${(sectionIndex++) * 0.1}s` }}>
                                    <SectionHeader title="Browse by Genre" count={genres.length} />
                        ...
                        <div className="px-6 py-4">
                            <div className="flex items-center gap-3 p-3 bg-white/[0.02] rounded-lg border border-white/5">
                                <div className="flex items-center gap-2 px-2.5 py-1 bg-[#1DB954]/10 rounded border border-[#1DB954]/20">
                                    ...
                                    <span className="text-[10px] font-black text-[#1DB954] uppercase tracking-wider">Spotify</span>
                                </div>
                                <div className="flex items-center gap-2 px-2.5 py-1 bg-[#a855f7]/10 rounded border border-[#a855f7]/20">
                                    ...
                                    <span className="text-[10px] font-black text-[#a855f7] uppercase tracking-wider">Deezer</span>
                                </div>
