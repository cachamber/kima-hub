...
import { X, Play, ListMusic, Waypoints, SkipForward } from "lucide-react";
import { useAudioControls } from "@/lib/audio-controls-context";
import type { MapTrack, PathResult, VibeMode, TrackResult } from "./types";
...
export function VibeInfoPanel({
    ...
    const isVisible = selectedTrack || similarTracks.length > 0 || pathResult;
        ...
        <div className="absolute right-0 top-0 bottom-0 w-80 bg-black/90 backdrop-blur-lg border-l border-white/10 flex flex-col z-20 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                <h3 className="text-sm font-medium text-white/90">
                    ...
                    {mode === "search" && selectedTrack && "Track Info"}
                    {mode === "path-result" && "Drift"}
            ...
            {selectedTrack && (mode === "idle" || mode === "search" || mode === "similar") && (
                <div className="px-4 py-3 border-b border-white/10">
                    ...
                    <div className="flex gap-2 mt-3">
                        ...
                        <button
                            ...
                            className="flex-1 px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded text-xs text-white/80 hover:text-white flex items-center justify-center gap-1"
                        >
                            <Waypoints className="w-3 h-3" /> Drift From Here
                        </button>
                    </div>
                </div>
            )}
            ...
            {similarTracks.length > 0 && (
                <div className="px-4 py-2 border-b border-white/10">
                    <button
                        onClick={() => handlePlayAll(similarTracks)}
                    ...
                    >
                        <Play className="w-4 h-4" /> Play All ({similarTracks.length})
                    </button>
                </div>
            )}
            ...
            {pathResult && mode === "path-result" && (
                <div className="px-4 py-2 border-b border-white/10">
                    <button
                        ...
                        className="w-full px-3 py-2 bg-white/10 hover:bg-white/15 rounded-lg text-sm text-white/80 hover:text-white flex items-center justify-center gap-2"
                    >
                        <Play className="w-4 h-4" /> Play Drift ({pathResult.metadata.totalTracks} tracks)
                    </button>
                </div>
            )}
